...
class EditableTableRow extends Component {
    constructor(props) {
        ...
        Object.keys(this.props.food)
            .filter((key) => key !== 'id')
            .forEach((key) => (key === 'nutrition_facts'
                ? Object.keys(this.props.food[key]).forEach(
                    (nutritionKey) => (this.state[
                ...
                : key === 'meal_set'
                    ? (this.state[key + this.props.food.id] = this.props.food[key]
                        ...
                        .reduce(
                            (total, actual, index, array) => (index === array.length - 1
                                ? total + actual
                                : `${total + actual}, `),
                            '',
                        ))
  ...
  render() {
      return (
          <Table.Row>
              {Object.keys(this.props.food)
                  .filter((key) => key !== 'id')
                  .map((key) => (key === 'measure_amount'
                      ? 'measure_type'
                      : key === 'measure_type'
                          ? 'measure_amount'
                          : key))
                  .map((key) => (key === 'nutrition_facts' ? (
